...
- 一个告诫是JavaScript中的一些 [“falsy” 值](https://developer.mozilla.org/en-US/docs/Glossary/Falsy)(比如数字`0`)，它们依然会被React渲染。当 `props.message` 为空数组时，它会打印`0`。
...
- JSX必须要有闭合标签，否则无法识别，不像html浏览器会自动加上
...
- 因为setState并不会立刻修改this.state的值，而且会对多个state进行合并，所以下面的code可能产生很不直观的结果：
...
+ 因为React会依次合并所有setState产生的效果，虽然前两个函数式setState调用产生的效果是count加2，但是半路杀出一个传统式setState调用，一下子强行把积攒的效果清空，用count加1取代。因为此时的this.state还没有被改变
...
+ 受控表单，如果value绑定的state没有初始值，那么value的值即是undefined，react会认为这个组件时非受控组件，一旦此时修改了state的值，相当于就是把非受控组件转换成了受控组件，react会报错
...
- 对于受控组件，请在state初始化的时候赋予默认值，但是无论是value初始化还是defaultValue都没有placeholder的效果，而且同时设置的时候也会覆盖placehloder的效果

- 解决：初始化state为空串，显式null、undefined也会报错
...
+ https://zh-hans.reactjs.org/docs/uncontrolled-components.html#the-file-input-tag
...
+ https://zh-hans.reactjs.org/docs/react-dom.html#finddomnode
...
+ react组件本身类似于js的函数，props就是我们传入的参数，组件本身严格遵守纯函数的概念，不改变传入的参数，在相同的参数下，总是输出相同的结果，但是我们又需要组件是动态的，于是就有了state**
...
+ 虚拟DOM标签中，html的静态属性写在开头，动态属性写在后面