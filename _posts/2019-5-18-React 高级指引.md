...
+ 使用redux把所有的状态都集中到一个container，然后通过context传递下去，这样就可以在一个组件内看到所有数据的流动，这样做好不好呢？
...
+ 第一个参数是被包裹的组件，第二个参数是一个函数，根据给定的`DataSource`和当前props属性中，返回所需要的数据
...
- ![1552564808864](F:\OneDrive\JS\assets\1552564808864.png) 
