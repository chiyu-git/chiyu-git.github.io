...
+ 在 PreviewListRedux.js 的最后，则是我们定义的 reducer。可以看到，我们的 reducer 会响应 3种类型的 action——LOAD_ARTICLES、LOAD_ARTICLES_SUCCESS 和 LOAD_ARTICLES_ERROR，这也是我们在之前刚刚定义的 action creator 可能会触发的 action 类型。 
...
+ 我们现在已经熟悉了views/文件夹和components/文件夹的职责区别。很显然，views/ HomeRedux.js 包含了 Home 页面所有组件相关的 reducer 及 actionCreator： 
...
+ ![1552218496074](F:\OneDrive\JS\纲略合集\assets\1552218496074.png) 
...
- ![1552263732335](F:\OneDrive\JS\纲略合集\assets\1552263732335.png) 
...
- ![1552264177448](F:\OneDrive\JS\纲略合集\assets\1552264177448.png) 
...
- 显而易见的变化是，我们在原本的标题中添加了链接，并指定了点击链接时由 handle Navigate方法来响应。在 handle Navigate 方法中，首先执行 e.prevent Default() 来阻止原始的链接跳转，
...
- ![1552264496803](F:\OneDrive\JS\纲略合集\assets\1552264496803.png) 
...
+ 两个Counter组件内部的数据是独立的，无法互相影响，状态管理也就无从说起。 究其原因，useReducer内部也是用useState实现的
...
+ 它接受一个由React.createContext返回的上下文对象， 当provider更新时，本文中这里理解为**传入的store更新时**，useContext就可以返回最新的值。那么我们就有了下面的代码
